...
func (h *ConversationHandler) RegisterRoutes(router gin.IRouter) {
	...
	router.GET("/conversations/:id/calls", h.CallHistory)
...
func (h *ConversationHandler) Update(c *gin.Context) {
	...
	var req httpdto.UpdateDisappearingModeRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.writeError(c, sentinal_errors.ErrInvalidInput)
		return
	}

	conv, err := h.service.UpdateDisappearingMode(ctx, services.UpdateDisappearingModeInput{
...
func (h *ConversationHandler) Delete(c *gin.Context) {
	...
	if err := h.service.DeleteForMe(ctx, services.DeleteConversationInput{ConversationID: conversationID, ActorID: userID}); err != nil {
		h.writeError(c, err)
		return
	}

	httpdto.WriteSuccess(c, http.StatusOK, httpdto.DeleteConversationPayload{Deleted: true})
}
...
func (h *ConversationHandler) CallHistory(c *gin.Context) {
	ctx := c.Request.Context()
	userID, ok := mustUserID(c)
	if !ok {
		h.writeError(c, sentinal_errors.ErrUnauthorized)
	...
	var query httpdto.CallHistoryQuery
	_ = c.ShouldBindQuery(&query)

	items, total, err := h.service.CallHistory(ctx, conversationID, userID, query.Page, query.Limit)
	if err != nil {
		h.writeError(c, err)
		return
	}

	httpdto.WriteSuccess(c, http.StatusOK, httpdto.ListPayload[services.CallHistoryItemView]{Items: items, Total: total})
}
