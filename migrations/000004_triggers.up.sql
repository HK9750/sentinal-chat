...
-- Auto-assign monotonic sequence number to new messages
...
-- ============================================================
-- View-once attachment redaction
-- ============================================================
DROP TRIGGER IF EXISTS tr_attachments_view_once ON attachments;
CREATE TRIGGER tr_attachments_view_once
    AFTER UPDATE OF viewed_at ON attachments FOR EACH ROW
    EXECUTE FUNCTION fn_mark_view_once();

-- ============================================================
-- Backfill: populate DM pairs from participants for existing data
-- ============================================================
WITH dm_pairs AS (
    SELECT
...
-- Deduplicate DM conversations (keep earliest, merge data)
...
duplicates AS (
    ...
    FROM dm_conversations WHERE rn > 1
),
survivors AS (
    SELECT conversation_id, user_id_a, user_id_b
    FROM dm_conversations WHERE rn = 1
...
-- Rebuild sequence counters from actual message data
